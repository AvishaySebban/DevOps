...
def main() :
   ...
   parser.add_argument("--uptime", action="store_true", help="verify the load usage")
   ...
   LOG_FILE=LOG_BASE_DIR + "/" + TEST_NAME + ".log" #LOG file name
   CSV_FILE=LOG_BASE_DIR + "/" + TEST_NAME + ".csv" #CSV file name
   ZIP_FILE=LOG_BASE_DIR + "/" + TEST_NAME + ".zip" #CSV file name
   ...
   if args.load or args.all:
     (exit_code_current,load)=checkLoad()
     if exit_code_current > EXIT_CODE:
       EXIT_CODE=exit_code_current
   ...
   if args.uptime or args.all:
     (exit_code_current,uptime)=checkUptime()
     if exit_code_current > EXIT_CODE:
       EXIT_CODE=exit_code_current

   #Print Sum:
   # uptime_usage - must be last
   csvLine=[i.year,i.month,i.day,i.hour,i.second,host_name,host_ip,str(EXIT_CODE),disk_usage,disk_free,cpu_usage,memo_usage,str(load),uptime]
   for ndx, member in enumerate(csvLine):
     csvLine[ndx] = str(member)+','
   writeToFile("".join(csvLine)+"\n",CSV_FILE)
   # uptime_usage - must be last
   print ("%s,%s,%s,%s:%s,%s,%s,%d,%s,%s,%s,%s,%s,%d days" %(i.year,i.month,i.day,i.hour,i.second,host_name,host_ip,EXIT_CODE,disk_usage,disk_free,cpu_usage,memo_usage,load,uptime))
   
  
   #Send severity mail alert
   if EXIT_CODE >= 2:
      create_Alert(ALERT_TYPE_CRITICAL,TEST_NAME,[LOG_FILE,CSV_FILE],ZIP_FILE)
   elif EXIT_CODE >= 1:
       create_Alert(ALERT_TYPE_WARNING,TEST_NAME,[LOG_FILE,CSV_FILE],ZIP_FILE)
